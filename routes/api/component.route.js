...
const download = require("../../middleware/download.middleware");
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const { name, description, url, images, size } = req.body;
			...
			if (size) componentFields.size = size;
...
router.put("/:id/like", auth, async (req, res) => {
	try {
		...
		for (let i = 0; i < component.likes.length; i++) {
			if (component.likes[i].toString() === req.user.id) {
		...
		if (!found) component.likes.unshift(req.user.id);
...
router.post(
	"/:id/rating",
	[auth, [check("stars", "should be a number").isNumeric()]],
	async (req, res) => {
		const errors = validationResult(req);
...
router.get("/:id/rating/:ratingId", async (req, res) => {
	try {
		...
		const rating = component.ratings.filter(
			(rating) => rating._id.toString() === req.params.ratingId.toString()
		);
...
router.delete("/:id/rating/:ratingId", auth, async (req, res) => {
	try {
		...
		component.ratings = component.ratings.filter(
			(rating) => rating._id.toString() !== req.params.ratingId.toString()
		);
...
//@route  GET api/component/downloads
//@desc  download component information
//@access private
router.put(
	"/:id/download",
	[
		[auth, download],
		[check("downloaded", "downloaded is required").isBoolean().notEmpty()],
	],
	async (req, res) => {
		try {
			const component = await Component.findOne({ _id: req.params.id });
			...
			if (req.body.downloaded) {
				component.downloadedBy.unshift(req.user.id);
			}

			component.save();
			return res.json(req.body.downloaded);
		} catch (error) {
			console.error(error.message);
			res.status(500).send("server error");
		}
